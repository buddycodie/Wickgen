...
export default function FitGenCore() {
  return (
    <div className="page-container bg-dark-bg text-gray-300 pt-28 pb-16 relative overflow-hidden">
      ...
      <div className="bg-slate-50 w-full border-t border-slate-200/50 py-8 text-slate-800 relative z-10">
        ...
        <section className="max-w-6xl mx-auto px-6 py-8 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            ...
            <h2 className="text-3xl font-bold text-slate-900 tracking-tight">A case study in mobile performance & telemetry.</h2>
          ...
          <div className="space-y-4">
            {features.map((f, i) => (
              ...
              >
                <div className="mt-1 bg-blue-50 p-2 rounded-lg shrink-0 text-blue-600 border border-blue-100">{f.icon}</div>
                <div>
      ...
      <div className="bg-white w-full border-t border-slate-200/50 py-16 text-slate-800 relative z-10">
        {/* Technologies Section */}
        <section className="max-w-6xl mx-auto px-6 relative z-10">
          <div className="text-center max-w-3xl mx-auto mb-12 space-y-4">
            <h2 className="text-2xl md:text-3xl font-bold text-slate-900 tracking-tight">FitGenCore Technology Stack</h2>
            <p className="text-slate-600 text-sm font-medium">We used a variety of modern protocols to structure FitGenCore.</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            {technologies.map((tech) => (
              <div key={tech.name} className="p-4 rounded-xl border border-slate-200/60 bg-slate-50 shadow-sm transition-all hover:shadow-md">
                <p className="font-semibold text-slate-900 text-sm">{tech.name}</p>
                <p className="text-[10px] text-slate-600 mt-1 leading-normal">{tech.desc}</p>
              </div>
            ))}
          </div>
        </section>
